...
test('account guide routes common requests to safe workspace pages', () => {
  assert.equal(suggestedAssistantPath('How do I connect Salesforce?'), '/apps');
  assert.equal(suggestedAssistantPath('Where should I rotate an API key?'), '/credentials');
...
test('account guide prompt preserves honest integration boundaries', () => {
  ...
  assert.match(prompt, /general-knowledge and business questions/);
  assert.match(prompt, /Lead with the direct answer/);
