...
import { Bot, GitBranch, Play, Save, Square, Type, X } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:12 }}>
        ...
        {['agent', 'transform', 'condition'].map(type => {
      ...
      {workflow?.status === 'active' && (
        <div style={{ ...inspector, marginTop:16 }}>
          ...
          {job && <p style={{ color:'#9CA3AF', fontSize:12, marginTop:10 }}>Job status: {job.status} · attempt {job.attempt || 0}</p>}
