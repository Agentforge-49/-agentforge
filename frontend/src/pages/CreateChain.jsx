  ...
  return (
    <div style={{ maxWidth:760, margin:'0 auto', color:'white', fontFamily:'system-ui, sans-serif' }}>
      ...
      <div style={{ marginBottom:24 }}>
        ...
        <p style={{ color:'#9CA3AF', fontSize:14 }}>Pick 2 or more agents. The first agent's answer automatically becomes the next agent's question.</p>
      </div>
      ...
      <div style={{ background:'#1A1D27', border:'1px solid #2A2D3E', borderRadius:16, padding:24 }}>
        ...
        <div style={{ marginBottom:16 }}>
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Research then Write" style={inputStyle} />
        </div>
        <div style={{ marginBottom:24 }}>
          <label style={{ display:'block', fontSize:13, color:'#9CA3AF', marginBottom:6 }}>Description (optional)</label>
          <input value={description} onChange={e => setDescription(e.target.value)} placeholder="What does this chain do?" style={inputStyle} />
        </div>
              ...
              <div key={agent.id} style={{ display:'flex', alignItems:'center', gap:10 }}>
                <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'space-between', background:'#0F1117', border:'1px solid #7C3AED55', borderRadius:10, padding:'10px 14px' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    <span style={{ width:22, height:22, borderRadius:'50%', background:'#7C3AED', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:600, flexShrink:0 }}>{i + 1}</span>
                    <span style={{ fontSize:13, fontWeight:500 }}>{agent.name}</span>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                    <button onClick={() => moveUp(i)} disabled={i === 0}
                      style={{ background:'transparent', border:'none', color: i === 0 ? '#374151' : '#9CA3AF', cursor: i === 0 ? 'default' : 'pointer', fontSize:14, padding:'2px 6px' }}>↑</button>
                    <button onClick={() => moveDown(i)} disabled={i === selected.length - 1}
                      style={{ background:'transparent', border:'none', color: i === selected.length - 1 ? '#374151' : '#9CA3AF', cursor: i === selected.length - 1 ? 'default' : 'pointer', fontSize:14, padding:'2px 6px' }}>↓</button>
          ...
          <>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:24 }}>
              {availableAgents.map(agent => (
                <button key={agent.id} onClick={() => addAgent(agent)}
                  style={{ display:'flex', alignItems:'center', gap:6, background:'#0F1117', border:'1px solid #2A2D3E', borderRadius:10, padding:'8px 14px', color:'white', cursor:'pointer', fontSize:13 }}>
        ...
        <button onClick={handleCreate} disabled={saving}
          style={{ width:'100%', background: saving ? '#5B21B6' : '#7C3AED', color:'white', border:'none', padding:14, borderRadius:12, fontSize:15, fontWeight:600, cursor: saving ? 'not-allowed' : 'pointer' }}>
